...
import { MapPin, Calendar, Mail, Sparkles, CheckCircle2, Eye, PlusCircle } from 'lucide-vue-next'
...
const formattedPreviewDate = computed(() => {
  if (!form.value.dateFound) return 'Date not specified'
  return new Date(form.value.dateFound).toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
    year: 'numeric'
  })
...
const validateForm = (): boolean => {
  if (!form.value.description.trim()) {
    error.value = 'Please describe the found item'
    return false
  }
  ...
  if (!form.value.locationFound.trim()) {
    error.value = 'Please enter where the item was found'
    return false
  }
  if (!form.value.dateFound) {
    error.value = 'Please enter the date found'
    return false
  }
  ...
  try {
    const { error: supabaseError } = await supabase
      ...
      .insert({
        ...
        location_found: form.value.locationFound.trim(),
        date_found: new Date(form.value.dateFound).toISOString(),
      ...
      <form @submit.prevent="submitForm" class="form-card card card-elevated">
        ...
        <div class="form-group">
          ...
          <textarea
            ...
            class="form-textarea"
            placeholder="Describe the item you found (e.g. dark wireless earbud case found beside the coffee shop)"
            rows="3"
            required
          ></textarea>
        </div>
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input
              id="locationFound"
              v-model="form.locationFound"
              type="text"
              class="form-input"
              placeholder="e.g. Coffee Shop Desk, Student Union"
              required
            />
          </div>
        </div>
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input
              ...
              type="text"
              class="form-input"
              placeholder="Where is the item held? (e.g. Library Front Desk)"
            />
          </div>
        </div>
        ...
        <div class="form-actions">
          <button type="submit" class="btn btn-found btn-lg" :disabled="isSubmitting">
            ...
            <span>{{ isSubmitting ? 'Submitting Report...' : 'Submit Found Report' }}</span>
          </button>
        </div>
      </form>
      ...
      <aside class="preview-sidebar">
        <div class="preview-sticky-box card card-elevated">
          <div class="preview-header">
            <Eye class="preview-icon text-found" />
          ...
          <div class="preview-report-card">
            ...
            <div class="preview-location">
              ...
              <span>{{ form.locationFound || 'Location not specified yet' }}</span>
            </div>

            <div class="preview-contact">
              <Mail class="icon-xs" />
              <span>{{ form.contactEmail || 'finder@university.edu' }}</span>
            </div>
          </div>
        </div>
      </aside>
